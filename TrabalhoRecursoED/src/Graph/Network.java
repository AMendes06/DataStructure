...
public class Network<T> extends MatrixGraph<T> implements NetworkADT<T> {
    ...
    public Object getVertice(int target) throws ElementNotFoundException {
        if (indexIsValid(target)) {
            Object vertex = vertices[target];
            return vertex;
        } else {
            throw new ElementNotFoundException("Empty Vertice");
        }
    }

    public boolean areConnected(int vertex1, int vertex2) throws ElementNotFoundException {
        return super.areConnected(vertex2, vertex2);
    }

    public void addEdgeWithTwoTypes(T vertex1, T vertex2, double weight) {
        addEdge(getIndex(vertex1), getIndex(vertex2), weight);
    }

    public void removeEdgeWithTwoTypes(T vertex1, T vertex2) {
        removeEdge(getIndex(vertex1), getIndex(vertex2));
    }
